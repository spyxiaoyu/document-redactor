  ...
  findSensitiveContent(text: string, options: FindOptions = {}): SensitiveDetectionResult {
    ...
    for (const rule of this.rules) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        // PHONE 切片 FP（第七批真合同 audit — fixture P7/P9 反复出现）：
        //   PHONE regex `0\d{2,4}[-.\s]?\d{7,8}` 会在更长的纯数字串中段切出 11-13 位"固话形态"子串
        //     - P7 "大额支付行号：12位CNAPS" → 切前一位"1"后的 "11位数字" 11 位
        //     - P9 "18位USCC"（mammoth 拼丢换行）→ 中段切 "13位数字" 13 位
        //   判别：真固话/手机号由 label（电话：）或空白分隔 → 紧邻前后不是数字
        //         切片必然紧贴其他数字（前或后是 digit）→ 拒
        //   回归安全：P1 "电话：11位固话"（前"："）/ "区号-号码"（前"："）/ 手机号（前"："）均不受影响
        if (rule.type === 'PHONE') {
        ...
        // BANK_CARD post-filter v2（spy 6-docx audit — fixture "19位畸形ID" FP）：
        ...
        //     - 19 chars, region 6位 + year 1990 前缀 → 排除 ✅
        //     - 19 chars, year 前缀不匹配 18|19|20 → 保留 ✅
        //     - 19 chars, 首字符 0 不匹配 [1-9] → 保留 ✅
        //     - 17 chars, year 前缀不匹配 → 保留 ✅
        //   阈值 17：ID_CARD 至少 17 位 prefix 才检查
        // v3 修复（spy 6-docx audit — fixture "17字typo身份证"）：
        //   原 16 位纯卡（如 "1234567812345678"）会被 BANK_CARD 识别，但若后接 X（ID 校验码）或
        //   数字（更长 ID 段），实际是 17+ 位身份证 typo，不是银行卡
        //   修法：value.length === 16 时检查下一字符
        //     - 16 位 + X → "16位ID前缀+X" → 拒 ✅ (fixture FP 修复)
        ...
        if (rule.type === 'NAME') {
          ...
          // v3 多姓名续接 FP 兜底 + 拆分 emit（spy — "联系人：张三、李四" 漏识别 + 真实合同叙述短语 FP）：
          //   regex 续接段贪婪吞 label 后文字或合同叙述短语
          //   修法：多姓名 match 拆成多个独立 SensitiveMatch，每个姓名 1 个
          //     - 拆分策略：value 按 [、，；] 切段，逐段验证，遇坏段就停
          //     - 坏段定义：①以"本"开头（合同指示代词）②匹配 label/动词前缀 ③含数字/英文/括号
          //     - 遇坏段立即停（不继续后续段，避免 1 坏段污染后续）
          //     - 单姓名场景行为不变（无分隔符 → 不进入这段）
          //   emit 方式：在 filter 中直接 push 到 matches，独立 start/end，绕过下方默认 push
          //     → 上游 UI 高亮/脱敏/还原都能按姓名独立处理
          if (/[、，；]/.test(value)) {
            ...
            // 扫描所有段，收集有效段（遇坏段就停）
            const validSegments: { text: string; offset: number }[] = [];
            const segmentRegex = /[^\s、，；]+/g;
            let segMatch: RegExpExecArray | null;
            while ((segMatch = segmentRegex.exec(value)) !== null) {
              if (isBadSegment(segMatch[0])) break;
              validSegments.push({ text: segMatch[0], offset: segMatch.index });
            }
            // 没收集到任何有效段 → 第一段就坏，整个 match 拒
            if (validSegments.length === 0) continue;
            // emit 每个有效段为独立 SensitiveMatch
            for (const seg of validSegments) {
              const segStart = start + seg.offset;
              matches.push({
                id: generateUUID(),
                type: rule.type,
                value: seg.text,
                ...
                context: extractContext(text, segStart, 30),
                blockId: undefined,
              });
            }
            // 跳过下方默认 push（已 emit）
        ...
        // PROJECT_NAME 叙述短语 FP（spy 合同 2026-07-27 FP-B）：
        ...
        if (rule.type === 'COMPANY') {
          ...
          // mammoth 双 cell 拼接 trim（第六批 audit — fixture "X有限公司公司账号"）：
...
if (!formMatch) continue;
          ...
          //   - "委托北京甲集团公司代理示例公司..." → 切"委托" → "北京甲集团公司代理示例公司..." (mid-verb reject 兜底)
          ...
          //     加 "）"/")"/"子公司"：case 47 "）子公司示例公司（上海）文化科技有限公司"
          //     切右括号(前一实体残留) + "子公司"通用前缀 → emit "示例公司（上海）文化科技有限公司" ✅
          //     真公司字号不以 "）"/")"/"子公司" 开头 → 不误伤
          //   - v7 cuttablePrefix 扩展（第六批 audit — fixture "剧目由北京占位传媒有限公司"）：
          ...
          // v4 例外（spy 6-docx audit - fixture "甲方为占位集团" 等真简称保留）：
          ...
          //   真公司名括号必配对（"示例公司（海南）..."/"示例公司（上海）..."）→ safeBody 括号数量不等即断裂残留 → 拒
          ...
          //     例 "）子公司示例公司（上海）..." 切"）"1字+扫"子公司"3字 → lastCutLength=3 → 剩"示例公司..." ≥3 → 保留 ✓
          if (safeStart > 0) {
          ...
          // 地址 FP 检查（fixture audit — "北京市示例区示例路（XX公司" 地址误识别）：
          //   - 原文是地址 "北京市示例区示例路（XX公司仓库）X号楼"
          //     COMPANY body 允许中文括号，从"北京市示例区示例路（XX"吞到第一个"公司" → 假公司名
          //   - 真公司 body 是"地名+品牌+行业词"，不含"行政区划+街道 token"完整地址链
          //     （"北京示例视频" 无 市/区/路；"中国示例银行股份" 无街道链 → 保留 ✅）
          //   - 判定：safeBody 含 (省|市|区|县) + 0-10 字 + (路|街|道|巷|弄) → 地址链 → 拒
          if (/(?:省|市|区|县)[\u4e00-\u9fa5（）()]{1,10}(?:路|街|道|巷|弄)/.test(safeBody)) continue;

          // 二次检查：safeBody 含合同叙述连词链 → 拒
          //   - 处理 case 3 "设计师及其所属"（"及其"是连词+代词链）
          //   - 处理 case 14 "示例双方的律师和顾问"（"和顾"是连词+名词链）
          //   - 处理 case 20 "设计师所属公司"（"所属"是代词+连词链）
          //   - v4 修复（spy — fixture "示例有限公司" / "和信科技" 误杀）：
          //     原 regex `[与和及其了在出于而之则这那每该各自己诸何属]` 单字类，safeBody 含任一单字即拒
          //     → 误杀真公司名含 connector 单字（fixture "示例己"含己、"和信"含和、"自庚"含自）
          //     改为具体 2 字连词链模式，覆盖原 case 3/14/20 全部 FP 场景但不误杀单字
          if (/(?:及其|所属|与各|和各|和代|和顾|和属|及代|及属|及顾|与代|与属|与顾|自己)/.test(safeBody)) continue;

          // 三次检查（顺位延续）：副词前缀拒（v3 spy 6-docx audit — fixture FP 修复）
          ...
          // "提供" coverb 叙述前缀 SPLIT（spy 合同 2026-07-27 FP-E）：
          ...
          //           例："次月20日前向甲方提供上月示例市场研究" → skip "上月" → "示例市场研究" 6 han ≥4 ✅ → emit
          ...
          //   trade-off：'提供' 在 body 首的真公司名仍被 ACTION_VERB_TRIGGERS 拒（既有 v3 行为，不引入 regression）
          {
            ...
            if (tigongIdx >= 2) {
              ...
              //   "提供上月示例市场研究" → skip "上月" → "示例市场研究" → 起始"示" → 真公司常用首字 → 切
          ...
          //   - "示例资本向" / "股权制定并经" / "且据集团" → end coverb → 拒
          ...
          //   2026-07-27 spy 合同反馈 FP-C："乙方作为代理公司" body "乙方作为代理" 以 "作为代理" 结尾
          ...
          // "以" coverb 叙述前缀 SPLIT（spy 合同 2026-07-27 FP-D）：
          ...
          //     用户可手动高亮两家公司
          //   - 阈值 18：单合法公司 body 通常 < 18 char（"示例公司（北京）融媒体科技文化有限" = 14 char）
 ...
 *   - 真公司名 body（"上海示例企业管理咨询"/"示例乳业"/"fixture网络科技"）都不含 trigger
 *
 * 排除词（避免误伤）：
 *   - "代理"（case 22 "智能代理有限公司"）
 *   - "发展"/"管理"/"控股"（行业词，真公司常用）
