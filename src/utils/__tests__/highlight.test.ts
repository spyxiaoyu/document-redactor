...
describe('buildHighlightParts: 取消高亮不能丢字段', () => {
  it('spy 截图 bug 重现：lastEnd 不推进会让 unselected match 范围内文本重复', () => {
    ...
    //   text = "甲：SAMPLE-CO-F乙：190740421030"
    //   match1 = "SAMPLE-CO-F"（已 selected）
    //   match2 = "190740421030"（用户刚点取消高亮 → unselected）
    //
    // 错误行为（bug）：unselected match 不推进 lastEnd，
    //   导致下一个普通文本区间重复包含老 match 范围
    //   → "甲：SAMPLE-CO-F乙：190740421030190740421030"（重复）
    //
    // 正确行为：unselected match 当 text 推进 lastEnd，parts 拼接后等于 text
    const text = '甲：SAMPLE-CO-F乙：190740421030';
    const matches: SensitiveMatch[] = [
      mkMatch('m1', 'COMPANY', 'SAMPLE-CO-F', 2),     // end=5
  ...
  it('多个 unselected match + selected match 混合，拼接仍等于原 text', () => {
    const text = '甲：SAMPLE-CO-F乙：SAMPLE-CO-Y丙：190740421030';
    const matches: SensitiveMatch[] = [
      mkMatch('m1', 'COMPANY', 'SAMPLE-CO-F', 2),
      mkMatch('m2', 'COMPANY', 'SAMPLE-CO-Y', 7),
      mkMatch('m3', 'BANK_CARD', '190740421030', 11),
    ];
    // 只有 m2 选中
  ...
  it('所有 match 都 selected 时，match 部分被替换为高亮（仍是 match 推进 lastEnd）', () => {
    const text = '甲：SAMPLE-CO-F乙：SAMPLE-CO-Y';
    const matches: SensitiveMatch[] = [
      mkMatch('m1', 'COMPANY', 'SAMPLE-CO-F', 2),
      mkMatch('m2', 'COMPANY', 'SAMPLE-CO-Y', 7),
    ];
  ...
  it('脱敏后面板（isOriginal=false）：selected 显示下划线占位，unselected 显示原文', () => {
    ...
    expect(m1Part!.text).toBe('\u00a0'.repeat(3));  // "SAMPLE-CO-F" 3 chars

    const reconstructed = joinText(parts);
    // 拼接结果：'甲：' + '   ' (3 NBSP) + '乙：' + 'SAMPLE-CO-Y' = '甲：   乙：SAMPLE-CO-Y'
    expect(reconstructed).toBe('甲：\u00a0\u00a0\u00a0乙：SAMPLE-CO-Y');
  });

  it('重叠 match（应由 SensitiveFinder merge 兜底）跳过，parts 不重复', () => {
    // match.start < lastEnd 视为重叠，跳过
    const text = 'SAMPLE-CO-F文化';
    const matches: SensitiveMatch[] = [
      mkMatch('m1', 'COMPANY', 'SAMPLE-CO-F文化', 0),  // 6 chars
