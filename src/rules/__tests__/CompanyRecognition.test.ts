...
describe('COMPANY 识别 — spy 截图 false positive 案例', () => {
  ...
  describe('真公司名必须能识别（不能误伤）', () => {
    it('case 4: "示例公司有限公司" — 完整型', () => {
      const text = '甲方为示例公司有限公司';
      const matches = findCompany(text);
      console.log(`\n[case 4] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
      expect(matches).toContain('示例公司有限公司');
    });
  ...
  describe('含连接词的应被拒', () => {
    ...
    it('case 14: "示例双方的律师和顾问公司" — body 含 "和"', () => {
      const text = '示例双方的律师和顾问公司';
      const matches = findCompany(text);
...
与这家公司和那个公司合作，每个公司都参与。`;
      ...
      expect(matches.some(m => m.includes('示例公司'))).toBe(true);
      // merged FP 整体不应出现
  ...
  describe('zcool 真实 docx 第二轮 FPs（probe 测试先行）', () => {
    ...
    it('case 21: mid-verb (委托) 长 body 应拆成两家公司（不是整体拒）', () => {
      ...
      const text = '北京示例数字技术有限公司委托北京甲集团网络科技有限公司承办';
      const matches = findCompany(text);
      console.log(`\n[case 21] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
      // split 期望：识别两家独立公司
      expect(matches.some(m => m === '北京示例数字技术有限公司')).toBe(true);
      expect(matches.some(m => m === '北京甲集团网络科技有限公司')).toBe(true);
      // merged FP 整体不应出现
      expect(matches.some(m => m.includes('北京示例数字技术有限公司委托'))).toBe(false);
    });
...
describe('AMOUNT_UPPER + BANK_CARD 大括号/前导 0 bug 修复', () => {
   ...
   *   - AMOUNT_UPPER 残缺匹配：fixture "180万元" 只 match "万元" 2 chars
   *     修法：regex 加 Arabic digit 前缀 alternation `\d+(?:,\d{3})*(?:\.\d+)?[万亿]元?`
   *   - BANK_CARD 误匹配畸形 ID：v3 \d{3,6} 让 19 位畸形 ID 卡也被识别为银行卡
  ...
  describe('AMOUNT_UPPER 阿拉伯数字前缀 + BANK_CARD 排除畸形 ID', () => {
    ...
    describe('BANK_CARD 应排除畸形 19 位 ID 卡格式', () => {
      it('case D1: 19 位畸形 ID → 不应被识别为 BANK_CARD', () => {
        // 修法：post-filter 排除 ID_CARD 格式（region(6) + 19|20 年份(4) + ...）
        const text = '主力编剧 张三 男 43062419900060600X';
        const matches = findByType(text, 'BANK_CARD');
        console.log(`\n[case D1] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m === '43062419900060600X')).toBe(false);
      });

      it('case D2: 19 位真银行卡 不应回归', () => {
        // 已 pass: 19 位银联卡
    ...
    describe('TAX_ID 中文括号 + 短 label variant', () => {
      // spy 6-docx audit - fixture bug：
      //   text: "纳税识别号：【XXXUSCCXXX】"
      ...
      it('case F1: "纳税识别号：【XXX】" 应识别 TAX_ID（5字 label + 中文左括号）', () => {
        const finder = new SensitiveFinder();
        const text = '公司名称：【上海示例传媒广告有限公司】\n纳税识别号：【1234567890ABCDEFGX】\n开户银行：【示例银行示例支行】';
        const result = finder.findSensitiveContent(text);
        ...
        expect(taxIds[0].value).toBe('1234567890ABCDEFGX');
      });
      ...
      it('case F3: "税号：（XXX）" 应识别 TAX_ID（全角圆括号）', () => {
        const finder = new SensitiveFinder();
        const text = '税号：（ABCDEFGHIJKLMNOPQ1）';
        const result = finder.findSensitiveContent(text);
        const taxIds = result.matches.filter(m => m.type === 'TAX_ID');
        console.log(`\n[case F3] 匹配 TAX_ID: ${JSON.stringify(taxIds.map(m => m.value))}`);
        expect(taxIds.length).toBe(1);
        expect(taxIds[0].value).toBe('ABCDEFGHIJKLMNOPQ1');
      });
    ...
    describe('BANK_CARD 应排除 ID_CARD 16 位前缀 + X 片段', () => {
      // spy 6-docx audit - fixture bug：
      //   text: "ABCD16位EFGH2X"（17 字 typo 缺一位身份证 + X 校验位）
      //   现状：BANK_CARD 只匹配前 16 位 "ABCD16位EFGH2"，原 X 不在 match 范围
      //   已有 post-filter 仅检查 length ≥17 → 16 位不挡 → 误识别为银行卡
      // 修法：post-filter 加 v3 - 16 位 + 后接 [X/x/数字] 视为 ID_CARD 片段排除

      it('case G1: 16 位 + X 应排除 BANK_CARD', () => {
        const finder = new SensitiveFinder();
        const text = '主力编剧 张三 女 ABCD16位EFGH2X 13800000001';
        const result = finder.findSensitiveContent(text);
        const banks = result.matches.filter(m => m.type === 'BANK_CARD');
        console.log(`\n[case G1] BANK_CARD 匹配: ${JSON.stringify(banks.map(m => m.value))}`);
        expect(banks.some(m => m.value === 'ABCD16位EFGH2')).toBe(false);
      });

      it('case G2: 16 位 + 数字 也应排除（继续延展为 ID 段）', () => {
        const finder = new SensitiveFinder();
        const text = '身份证 ABCD16位EFGH212345678';
        const result = finder.findSensitiveContent(text);
        const banks = result.matches.filter(m => m.type === 'BANK_CARD');
    ...
    describe('fixture-A 地址误识别为 COMPANY FP 修复', () => {
      // 根因：原文是地址 "北京市示例区示例路（XX公司仓库）X号楼"
      //   COMPANY regex body 允许中文括号，从"北京市示例区示例路（XX"吞到第一个"公司"
      //   → 假公司名 "北京市示例区示例路（XX公司"
      // 修法：body 含 行政区划(省/市/区/县) + 街道 token(路/街/道/巷/弄) → 判地址 FP 拒
      it('case 40: 地址"北京市示例区示例路（XX公司仓库）..." → 不应识别为 COMPANY', () => {
        const text = '地址：北京市示例区示例路（XX公司仓库）X号楼X层';
        const matches = findCompany(text);
        console.log(`\n[case 40] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.includes('信托公司'))).toBe(false);
        expect(matches.some(m => m.includes('示例路'))).toBe(false);
      });

      it('case 41 (regression): "北京示例视频科技有限公司" → 真公司应保留', () => {
        const text = '甲方：北京示例视频科技有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 41] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches).toContain('北京示例视频科技有限公司');
      });
    ...
    describe('产权承诺函 audit — 连词"因"前缀误吞 FP 修复', () => {
      // 根因：原文 "因示例公司（海南）融媒体科技有限公司编制财务报告需要..."
      //   "因"是连词（因…需要…），被吞进 COMPANY body 开头 → 假公司名 "因示例公司（海南）融媒体科技有限公司"
      // 修法：cuttablePrefix 加单字连词 "因"（与现有 "经" 同类），切后 emit 真公司名
      it('case 43: "因示例公司（海南）融媒体科技有限公司编制..." → 切"因"保留真公司名', () => {
        const text = '因示例公司（海南）融媒体科技有限公司编制财务报告需要拟对其并购';
        const matches = findCompany(text);
        console.log(`\n[case 43] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.startsWith('因'))).toBe(false);
        expect(matches).toContain('示例公司（海南）融媒体科技有限公司');
      });
    ...
    describe('有关事项说明(资产评估报告) audit — 叙述前缀/纯form/右括号 FP 修复', () => {
      ...
      //   陷阱：body 含"评估"的是真公司（"北京示例资产评估有限公司"）→ "评估"不能进动词表
      it('case 45: "本次评估对象为示例公司（海南）融媒体科技有限公司" → 叙述前缀不识别', () => {
        const text = '本次评估对象为示例公司（海南）融媒体科技有限公司';
        const matches = findCompany(text);
      ...
      it('case 47: "）子公司示例公司（上海）文化科技有限公司" → 切右括号+子公司前缀', () => {
        const text = '）子公司示例公司（上海）文化科技有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 47] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.startsWith('）') || m.startsWith('子公司'))).toBe(false);
        expect(matches).toContain('示例公司（上海）文化科技有限公司');
      });
      ...
      it('case 49 (regression 陷阱): "北京示例资产评估有限公司" → 评估公司必须保留', () => {
        const text = '受托方：北京示例资产评估有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 49] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches).toContain('北京示例资产评估有限公司');
      });
      ...
      it('case 53: "评估范围为示例公司（海南）融媒体科技有限公司" → 切叙述前缀（评估公司不误杀）', () => {
        const text = '评估范围为示例公司（海南）融媒体科技有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 53] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.includes('范围'))).toBe(false);
      });

      it('case 54: "政府补助为示例公司（上海）文化科技有限公司" → 切叙述前缀', () => {
        const text = '政府补助为示例公司（上海）文化科技有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 54] 输入: "${text}" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.includes('补助') || m.includes('政府'))).toBe(false);
      });

      // 第四批 audit（fixture-A/达人合作/演员录制） — 5 个新 FP 修复
      ...
      it('case 57: "上述甲方包括示例集团的成员单位（宁波示例..." → 叙述词"上述/包括"应拒', () => {
        const text = '上述甲方包括示例集团的成员单位（宁波示例厨具有限公司';
        const matches = findCompany(text);
     ...
     *   - 结果发现以下 6 类 FP（跨 3 个 docx：fixture-A/代销协议/演员录制）：
     *     ① COMPANY "剧目由北京占位传媒..." "剧目由" coverb 前缀漏切
     *     ② COMPANY "的独家经纪公司或代理公司" 描述性短语被识别
     *     ③ COMPANY "XX影院管理有限公司公司" mammoth 双公司拼接（docx 表格 cell merge 后 "公司   公司账号" 被吃成 "公司公司账号"）
     *     ④ BANK_CARD 18 位纯数字 统一社会信用代码（USCC 没 letter）被误吃
    ...
    describe('第六批 audit 全目录扫描 — 6 类新 FP', () => {
      ...
      it('case 64: COMPANY "）的独家经纪公司或代理公司" → 描述性短语应拒', () => {
        // 演员录制 真实上下文：'...以下简称"乙方艺人"）的独家经纪公司或代理公司，甲方拟邀请...'
        // regex 从 "）" 起匹配 → cuttablePrefix 切 "）" → safeBody "的独家经纪公司或代理"（以助词"的"开头）
        // 真公司 body 绝不以助词"的"开头 → NARRATIVE_BOUNDARY_VERB_START ^的 → 拒
        const text = '1234567890ABCDEFGX，以下简称"乙方艺人"）的独家经纪公司或代理公司，甲方拟邀请乙方艺人参与节目录制';
        const matches = findCompany(text);
        console.log(`\n[case 64] 输入: "...）的独家经纪公司或代理公司..." → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.includes('独家经纪公司或代理公司'))).toBe(false);
      });

      it('case 64b (回归): "示例家电集团有限公司" body 中段"的"不应被误伤', () => {
        // NARRATIVE_BOUNDARY_VERB_START 只查首字符 ^的，"示例" 的 "的" 在中段
        const text = '客户示例家电集团有限公司';
        const matches = findCompany(text);
        console.log(`\n[case 64b] 输入: "示例家电集团有限公司" → 匹配: ${JSON.stringify(matches)}`);
        expect(matches.some(m => m.includes('示例家电集团'))).toBe(true);
      });

      it('case 65: COMPANY mammoth 双公司拼接 "X有限公司公司" → 应拒（mammoth ate space）', () => {
        // 代销协议 [5896-5913] FP = "XX影院管理有限公司公司"
        // 真实 docx 内容: "公司名称：XX影院管理有限公司\n公司账号：..."
        // mammoth 双 cell 拼接中间空格消失 → "X有限公司  公司账号" → regex 匹配 "X有限公司公司"
        const text = '公司名称：北京示例影院管理有限公司公司账号：318156021864';
        const matches = findCompany(text);
      ...
      it('case 66: BANK_CARD 18 位纯数字 USCC（无 letter）→ 应拒', () => {
        // 代销协议 [3285-3305] 等 FP — 18 位纯数字 USCC（无 letter）
        // 真 USCC 末位是字母，docx mammoth 拼接丢了 letter
        // 区分 18+ 位纯数字 USCC（不会是银行卡 — 银行卡16-19位有 Luhn 校验或随机）
        const text = '示例影院管理有限公司ABCDEFGHIJKLMNOP1Q北京市示例区示例路';
        const finder = new SensitiveFinder();
        const result = finder.findSensitiveContent(text);
        const banks = result.matches.filter(m => m.type === 'BANK_CARD');
        console.log(`\n[case 66] BANK_CARD 匹配: ${JSON.stringify(banks.map(m => m.value))}`);
        expect(banks.some(m => m.value === 'ABCDEFGHIJKLMNOP1Q')).toBe(false);
      });

      it('case 67: NAME "联系电话"（4字 label）→ 应拒', () => {
        // fixture-A [8468-8472] FP — 上一批只挡了 3字短 label
